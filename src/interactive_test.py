...
def upload_pdf_interactive() -> None:
    ...
    # Ask for PDF path or URL
    pdf_source = input("Digite o caminho do arquivo PDF ou URL: ")

    if pdf_source.startswith(("http://", "https://")):
        # Download PDF from URL
        print(f"Baixando PDF de {pdf_source}...")
        saved_path = download_pdf_from_url(pdf_source)

        if saved_path:
            print_colored(f"PDF baixado e salvo em: {saved_path}", "green")
        else:
            print_colored("Erro ao baixar o PDF", "red")
    else:
        ...
        try:
            if os.path.exists(pdf_source):
                ...
                saved_path = upload_pdf(pdf_data, filename)

                print_colored(f"PDF enviado e salvo em: {saved_path}", "green")
            else:
                print_colored(f"Arquivo não encontrado: {pdf_source}", "red")
        except Exception as e:
            print_colored(f"Erro ao enviar o PDF: {str(e)}", "red")
